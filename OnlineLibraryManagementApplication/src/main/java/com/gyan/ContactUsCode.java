...
@WebServlet("/ContactUsCode")
public class ContactUsCode extends HttpServlet {
	private static final long serialVersionUID = 1L;
	final static DateTimeFormatter CUSTOM_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
   
    public ContactUsCode() {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(num>0)
		{
			out.println("<script>window.location.href='index.jsp';alert('Thanks for contacting, We will meet you soon')</script>");
		}
		else {
			out.println("<script>window.location.href='contact.jsp';alert('Failed! Please Enter Valid Details')</script>");
		}
